...
class panopto_data {
     ...
     * @param string $capability The capability being given to the roles
     */
    public static function build_context_capability_to_roles($context, $roles, $capability) {
        $assigncaps = array();
        foreach ($roles as $role) {
            if (isset($role) && trim($role) !== '') {
    ...
    public static function build_and_assign_context_capability_to_roles($context, $roles, $capability) {
        ...
        $processed = false;
        $assigned = self::build_context_capability_to_roles($context, $roles, $capability);
        $existing = array();

        // Extract the existing capabilities that have been assigned for context, role and capability.
        foreach ($roles as $key => $roleid) {
            if ($DB->record_exists('role_capabilities', array('contextid'=>$context->id, 'roleid'=>$roleid, 'capability'=>$capability))) {
    ...
    /**
     * Build list of capabilities to be removed for a specified roles given a context.
     *
     * @param int $context the context of the roles being affected
     * @param array $roles an array of roles to be unnassigned
     * @param string $capability The capability being removed to the roles
     */
    public static function build_remove_context_capability_from_roles($context, $roles, $capability) {
        $unassigncap = array();
        foreach ($roles as $role) {
            $unassigncap[$role->shortname] = $capability;
        }
        return $unassigncap;
    }
